...
public class FuelGainMultiplyUpgrade extends Upgrade {

    public FuelGainMultiplyUpgrade(String name, int imageId, int cost, String description) {
    ...
    private void raisePrice() {//increase price after buying upgrade
        this.setCost(
                this.getCost() +
                        this.getMinCost() * GameProgress.getInstance().getFuelGainMultiply());
    }

    @Override
    public void buyItem(int money) {
        //checks if upgrade isn't MAXed
        if (GameProgress.getInstance().canBuyFuelGainMultiply()) {
            //if not MAXed return true to ShopActivity and spend money
            GameProgress.getInstance().buyFuelGainMultiply();
            super.buyItem(money);
