 ...
 */
public class HealGainMultiplyUpgrade extends Upgrade {

    public HealGainMultiplyUpgrade(String name, int imageId, int cost, String description) {
    ...
    private void raisePrice() {//increase price after buying upgrade
        this.setCost(
                this.getCost() +
                        this.getMinCost() * GameProgress.getInstance().getHealthGainMultiply());
    }

    @Override
    public void buyItem(int money) {
        //checks if upgrade isn't MAXed
        if (GameProgress.getInstance().canbuyHealthGainMultiply()) {
            //if not MAXed return true to ShopActivity and spend money
            GameProgress.getInstance().buyHealthGainMultiply();
            super.buyItem(money);
